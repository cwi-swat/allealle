...
Binding \join(1, 2, Binding lhs, Binding rhs) {
	Formula consVal(Index row) = \or({\and({lhs[<x>], rhs[y]}) | <Atom x> <- lhs, lhs[<x>] != \false(), Index y:<x, row> <- rhs, rhs[y] != \false()});
	return (<row>:val | <Atom row> <- lhs, Formula val := consVal(row), val != \false());
}

Binding \join(2, 1, Binding lhs, Binding rhs)  
	= (<row>:\or({\and({lhs[y], rhs[<x>]}) | Index y:<row, Atom x> <- lhs, lhs[y] != \false(), <x> in rhs, rhs[<x>] != \false()}) | <Atom row, _> <- lhs);
			
Binding \join(2, 2, Binding lhs, Binding rhs) 
	= (idx:(\false() | \or({it, \and({lhs[<row,x>], rhs[<x,col>]})}) | <row, Atom x> <- lhs, lhs[<row,x>] != \false(), <x,col> in rhs, rhs[<x,col>] != \false()) | Index idx:<Atom row, Atom col> <- lhs);
