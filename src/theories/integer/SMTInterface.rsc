...
Maybe[SMTVar] constructSMTVar(intVar(str name)) = just(<"<name>_int", integers()>);
...
Formula getValue(str smtValue, SMTVar var) = toFormula(smtValue) when var.theory == integers();
...
Formula mergeModel(Model foundValues, intVar(str name)) = foundValues[<"<name>_int", integers()>] when <"<name>_int", integers()> in foundValues;
