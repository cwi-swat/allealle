module theories::tests::binderTests::JoinTester

extend theories::tests::binderTests::TesterBase;

test bool test1x2Join_onlyTruthValues() {
	RelationMatrix person = t(["j"]) + t(["s"]) + t(["l"]);
	RelationMatrix parent = t(["j","l"]) + t(["s","l"]); 
 
	return 	\join(person, parent, voidAdder) == t(["l"]);
}
...
test bool test2x1Join_onlyTruthValues() {
	...
	return 	\join(parent, person, voidAdder) == t(["j"])+t(["s"]);				 
}
...
test bool test1x2Join_withVars() {
	...
	return 	\join(person, parent, voidAdder) == val(["l"], \or({var("j_l"), var("s_l")}));
}
